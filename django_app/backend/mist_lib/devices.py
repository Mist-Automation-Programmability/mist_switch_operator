...
# body: site_id, device_id, device_name, device_role, device_model
    ...
    def _get_device_settings(self, extract, body, retry=False):
        ...
        try:
            ...
            if retry:
                ...
                self._get_device_settings(extract, body, True)
    ...
    def _get_device_stats(self, extract, body, retry=False):
        try:
            ...
            if retry:
                ...
                self._get_device_stats(extract, body, True)
    ...
    def _get_site_template(self, extract, body, retry=False):
        try:
            ...
            if retry:
                ...
                self._get_site_template(extract, body, True)
    ...
    def _gen_fpc(self, port_type, mso_fpc, mso_pic, mso_slot, mist_fpc, mist_pic, mist_slot):
        ...
        if data["a"]:
            ports.append("{0}-{1}/{2}/{3}".format(port_type,
                                                  data["a"], mist_pic, mist_slot))
        if data["b"]:
            ports.append("{0}-{1}/{2}/{3}".format(port_type,
                                                  data["b"], mist_pic, mist_slot))
        ports += self._gen_pic(port_type, mso_fpc, mso_pic,
                               mso_slot, mist_pic, mist_slot)
        return ports
    ...
    def _gen_pic(self, port_type, fpc, mso_pic, mso_slot, mist_pic, mist_slot):
        ...
        if data["a"]:
            ports.append("{0}-{1}/{2}/{3}".format(port_type,
                                                  fpc, data["a"], mist_slot))
        if data["b"]:
            ports.append("{0}-{1}/{2}/{3}".format(port_type,
                                                  fpc, data["b"], mist_slot))
