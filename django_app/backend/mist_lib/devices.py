...
port_usage_name_prefix = "mso_autogenerated"
...
# body: site_id, device_id, device_name, device_role, device_model
    ...
    def _mutate_device_settings(self, body, mist_device_settings):
        for port in body["port_config"]:
            mist_device_settings = self._unset_port_conf(port, mist_device_settings)
            mist_device_settings = self._set_port_usage(port, mist_device_settings)
            mist_device_settings = self._set_port_conf(port, mist_device_settings)
        return mist_device_settings

    def _set_port_usage(self, port, mist_device_settings):        
        port_usage_name = "{0}_{1}".format(port_usage_name_prefix, port["port"])
        mist_device_settings["port_usages"][port_usage_name] = port["new_conf"]
        return mist_device_settings
            
    def _set_port_conf(self, port, mist_device_settings):        
        port_name = port["port"]
        port_usage_name = "{0}_{1}".format(port_usage_name_prefix, port_name)
        ...
        # else, need more validation
            ...
            for port_config in mist_device_settings["port_config"]:  
                ...
                for port_config_item in port_config_split:                        
                    ...
                    # if not a single port, means it's a port range
                        ...
                        if mist_port_type == mso_port_type and self._is_in(mso_fpc, mist_fpc) and self._is_in(mso_pic, mist_pic) and self._is_in(mso_slot, mist_slot):
                            tmp = self._gen_fpc(mist_port_type, mso_fpc, mso_pic, mso_slot, mist_fpc, mist_pic, mist_slot)
                            remaining_ports += tmp
                ...
                if not port_config == remaining_ports:
                    mist_device_settings["port_config"][remaining_ports] = mist_device_settings["port_config"].pop(port_config)
                    break
        return mist_device_settings
    ...
    def _gen_slot(self, port_type, fpc, pic, mso_slot, mist_slot):
        ...
        if data["a"]:
            ports.append("{0}-{1}/{2}/{3}".format(port_type, fpc, pic, data["a"]))
        if data["b"]:
            ports.append("{0}-{1}/{2}/{3}".format(port_type, fpc, pic, data["b"]))        
        return ports
