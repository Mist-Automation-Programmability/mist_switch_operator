...
class Devices(Common):
    """
    Manage Devices requests and content generation
    """
    #############
...
# body: site_id, device_id, device_name, device_role, device_model
    ...
    def get_device_settings(self, body):
        ...
        if not "status" in device_settings and not "status" in site_settings:
            data = self._generate_device_settings(
                extract, device_settings, site_settings, device_stats)
        ...
        try:
            ...
            if retry:
                ...
                return {
                    "status": 500, 
                    "data": {
                        "message": f"Unable to retrieve information for device {body['device_id']}"
                        }
                    }
            else:
    ...
    def _get_device_models(self, extract, retry=False):
        try:
            logging.info(f"Device:Retreiving Device Models: In Process")
            url = f"https://{extract['host']}/api/v1/const/device_models"
            resp = requests.get(
                url, headers=extract["headers"], cookies=extract["cookies"])
            logging.info(f"Device:Retreiving Device Model: Done")
            return resp.json()
        except:
            if retry:
                logging.error(f"Device:Retreiving Device Models: Error")
                return []
            else:
    ...
    def _generate_device_settings(self, extract, device_settings, site_settings, device_stats):
        data = {
            "members": [],
            "ports": {},
            "info":{
        ...
        for member in device_stats.get("module_stat", []):
            ...
            device_models = self._get_device_models(extract)
                ...
                for pic, interfaces in dev_pics.items():
                    ...
                    for int_def in interfaces_split:
                        ...
                        for int_num in range(i, int_count+i):
                            ...
                            interface_num = f"-{fpc}/{pic}/{int_num}"
                            if not interface_num in interfaces_done:
                                interfaces_done.append(interface_num)
                                tmp["ports"].append(interface_name)
                                data["ports"][interface_name] = {
                                    "port": interface_name, "site": {}, "device": {}}
                        i += int_count

            # Generate the list of ports based on the Device Stats
            if member.get("pics"):
                for pic in member.get("pics", []):
                    port_groups = pic.get("port_groups", [])
                    pic_index = pic.get("index")
                    int_num = 0
                    for port_group in port_groups:
                        port_group_count = port_group.get("count", 0)
                        port_group_type = port_group.get("type", "GE")
                        int_type = "ge"
                        if port_group_type.lower().startswith("sfp"):
                            int_type = "xe"
                        elif port_group_type.lower().startswith("qsfp"):
                            int_type = "et"
                        for int_num in range(int_num + 1, int_num + port_group_count + 1):
                            interface_name = f"{int_type}-{fpc}/{pic_index}/{int_num - 1}"
                            interface_num = f"-{fpc}/{pic_index}/{int_num - 1}"
                            if not interface_num in interfaces_done:
                                interfaces_done.append(interface_num)
                                tmp["ports"].append(interface_name)
                                data["ports"][interface_name] = {
    ...
    def _unset_port_conf(self, port:dict, mist_device_settings:dict):
        port_name = port["port"]
        # only useful when there are port configuration at the device level
        if "port_config" in mist_device_settings:
            # if a dedicated entry exists for this switchport, remove it from the config
